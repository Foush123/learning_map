...
function xmldb_learningmap_upgrade($oldversion) {
    ...
    // Add automode field to learningmap table.
    if ($oldversion < 2025093001) {
        $table = new xmldb_table('learningmap');
        $field = new xmldb_field('automode', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'showmaponcoursepage');

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_mod_savepoint(true, 2025093001, 'learningmap');
    }
